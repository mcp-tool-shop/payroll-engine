-- Phase 1 Payroll Platform — Postgres DDL with Required Deltas
-- Based on phase1_pack/ddl.sql with idempotency + locking additions
...
CREATE TABLE pay_run (
  ...
  -- DELTA: Add as_of_date for explicit date basis (defaults to period_end)
  as_of_date DATE,
  -- DELTA: Track if reopen happened
...
CREATE TABLE pay_run_employee (
  ...
  currency CHAR(3) NOT NULL DEFAULT 'USD' CHECK (currency = 'USD'),
  -- DELTA: Store error details when status=error
  error_message TEXT,
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
...
CREATE TABLE pay_statement (
  ...
  -- DELTA 1.1: One statement per run_employee
  -- DELTA 1.2: Calculation identity for idempotency
  calculation_id UUID NOT NULL,
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  CONSTRAINT pay_statement_one_per_pre UNIQUE (pay_run_employee_id)
);
...
CREATE TABLE pay_line_item (
  ...
  line_type TEXT NOT NULL CHECK (line_type IN ('EARNING','DEDUCTION','TAX','EMPLOYER_TAX','REIMBURSEMENT','ROUNDING')),
...
-- DELTA 1.2: Unique index for idempotent line item insertion
...
CREATE TABLE time_entry (
  ...
  approved_at TIMESTAMPTZ,
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  -- DELTA 2.1: Locking columns for approval freeze
  locked_by_pay_run_id UUID REFERENCES pay_run(pay_run_id),
...
CREATE INDEX time_entry_locked_idx ON time_entry(locked_by_pay_run_id) WHERE locked_by_pay_run_id IS NOT NULL;
...
CREATE INDEX pia_locked_idx ON pay_input_adjustment(locked_by_pay_run_id) WHERE locked_by_pay_run_id IS NOT NULL;

-- ===== 10.5) Pay run lock tracking (for config snapshots) =====
CREATE TABLE pay_run_lock (
  pay_run_lock_id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  pay_run_id UUID NOT NULL REFERENCES pay_run(pay_run_id) ON DELETE CASCADE,
  entity_type TEXT NOT NULL,
  entity_id UUID NOT NULL,
  snapshot_hash TEXT NOT NULL,
  locked_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  UNIQUE (pay_run_id, entity_type, entity_id)
);
CREATE INDEX pay_run_lock_run_idx ON pay_run_lock(pay_run_id);
...
CREATE TABLE payment_batch (
  ...
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  -- DELTA 1.3: One batch per run+processor
  CONSTRAINT payment_batch_one_per_run UNIQUE (pay_run_id, processor)
);
...
-- Prevent UPDATE/DELETE on pay_statement when pay_run is committed/paid
CREATE OR REPLACE FUNCTION prevent_statement_modification()
RETURNS TRIGGER AS $$
DECLARE
  run_status TEXT;
BEGIN
  ...
  WHERE pre.pay_run_employee_id = OLD.pay_run_employee_id;

  IF run_status IN ('committed', 'paid') THEN
    RAISE EXCEPTION 'Cannot modify pay_statement when pay_run is %', run_status;
  END IF;
...
CREATE TRIGGER trg_prevent_statement_modification
  BEFORE UPDATE OR DELETE ON pay_statement
  FOR EACH ROW
  EXECUTE FUNCTION prevent_statement_modification();

-- Prevent UPDATE/DELETE on pay_line_item when pay_run is committed/paid
...
BEGIN
  ...
  WHERE ps.pay_statement_id = OLD.pay_statement_id;

  IF run_status IN ('committed', 'paid') THEN
    RAISE EXCEPTION 'Cannot modify pay_line_item when pay_run is %', run_status;
  END IF;
...
CREATE TRIGGER trg_prevent_line_item_modification
  BEFORE UPDATE OR DELETE ON pay_line_item
  FOR EACH ROW
  EXECUTE FUNCTION prevent_line_item_modification();

-- Prevent modification of locked time entries
CREATE OR REPLACE FUNCTION prevent_locked_time_entry_modification()
RETURNS TRIGGER AS $$
BEGIN
  IF OLD.locked_by_pay_run_id IS NOT NULL THEN
    RAISE EXCEPTION 'Cannot modify time_entry locked by pay_run %', OLD.locked_by_pay_run_id;
  END IF;
...
CREATE TRIGGER trg_prevent_locked_time_entry_modification
  BEFORE UPDATE OR DELETE ON time_entry
  FOR EACH ROW
  EXECUTE FUNCTION prevent_locked_time_entry_modification();

-- Prevent modification of locked pay input adjustments
CREATE OR REPLACE FUNCTION prevent_locked_adjustment_modification()
RETURNS TRIGGER AS $$
BEGIN
  IF OLD.locked_by_pay_run_id IS NOT NULL THEN
    RAISE EXCEPTION 'Cannot modify pay_input_adjustment locked by pay_run %', OLD.locked_by_pay_run_id;
  END IF;
...
CREATE TRIGGER trg_prevent_locked_adjustment_modification
  BEFORE UPDATE OR DELETE ON pay_input_adjustment
  FOR EACH ROW
  EXECUTE FUNCTION prevent_locked_adjustment_modification();
